...
Flow: validate workspace/proxy/mailbox -> resume an unfinished child or reuse eligible standby -> claim one HME only when neither exists -> send and verify the requested invitation -> register at `https://chatgpt.com/` when `BROWSER_SIGNUP_FLOW=extension` (the legacy runner still uses the HTTPS invitation link) -> keep the browser and page open while confirming official membership/role/seat -> open the OAuth link in that same page (signup disabled) -> conditionally verify SMS -> validate callback and token email -> save access/refresh tokens.
...
- For Chromix, download and verify a trusted platform build separately. Set `BROWSER_ENGINE=chromix` and `BROWSER_EXECUTABLE` to its actual executable inside the runtime/container (for example `/app/data/browsers/chromix/chrome`). This enables a saved per-account seed, viewport and regional settings for registration, OAuth and standalone reauthorization. `--browser-executable` overrides the executable for one CLI run. It does not select the engine. Without Chromix mode, existing Playwright/browser-channel defaults apply. Both invitation stages retain the same running browser, profile and proxy bridge. See [browser environments](browser-environments.md) for profile persistence, rollout and binary compatibility.
