...
class UIContractTests(unittest.TestCase):
    ...
    def test_console_has_one_product_and_no_legacy_assets(self):
        with tempfile.TemporaryDirectory() as tmp, make_client(Path(tmp)) as client:
            ...
            css = client.get("/static/css/management.css").text
            self.assertIn(".management-selection[hidden]", css)
            ...
            self.assertIn("接入", js)
            # Legacy /workspaces list, portfolio and quota-meter rendering were removed in UI batch 6;
            # the accounts page (accounts-view.js) owns those views.
            self.assertNotIn("function renderPortfolio", js)
            ...
            self.assertIn("邀请加入团队", js)
            self.assertIn("补充团队", js)
            self.assertIn("/api/workspaces/${workspace.id}/replenish", js)
            self.assertIn("renderTeamReplenishControls", js)
            self.assertIn('new Option("团队默认", "workspace_default")', js)
            self.assertIn('seat_intent: values?.seat_intent || "workspace_default"', js)
            self.assertNotIn("官方席位固定邀请 Premium", js)
            self.assertNotIn("askReplenishPhoneLine", js)
            self.assertIn("改成所有者", js)
            self.assertIn("改成成员", js)
            self.assertIn("永久删除本地档案", js)
            self.assertIn("移出官方团队", js)
            self.assertIn("撤销官方邀请", js)
            self.assertNotIn('secondaryIds: ["team.member.role", "team.member.official-remove", "team.member.local-remove"]', js)
    ...
    def test_settings_never_returns_raw_secret(self):
        with tempfile.TemporaryDirectory() as tmp, make_client(Path(tmp)) as client:
            ...
            self.assertIn('name="auto_rotate"', settings_form)
            self.assertIn('name="auto_rotate_daily_limit"', settings_form)
            self.assertNotIn('name="force_refill"', settings_form)
            self.assertNotIn('id="rotate-sheet"', page)
            self.assertIn("先处理异常", client.get("/").text)
