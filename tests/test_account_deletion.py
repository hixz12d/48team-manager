...
class AccountDeletionTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def reject(self, code, email='unused@example.com'):
        with self.assertRaises(AccountDeletionError) as error:
            await delete_unassigned_account(self.db, self.account_id, email)
    ...
    async def test_local_delete_keeps_other_account_audit_and_hme_occupancy(self):
        ...
        await self.db.commit()
        result = await delete_unassigned_account(self.db, self.account_id, ' UNUSED@example.com ')
        await self.db.commit()
    ...
    async def test_wrong_confirmation_does_not_delete(self):
        await self.reject('confirmation_mismatch', 'wrong@example.com')
    ...
    async def test_removed_membership_can_be_deleted(self):
        ...
        await self.db.commit()
        await delete_unassigned_account(self.db, self.account_id, self.account.email)
        await self.db.commit()
    ...
    async def test_pending_oauth_blocks_and_expired_session_is_removed(self):
        ...
        await self.db.commit()
        await delete_unassigned_account(self.db, self.account_id, 'unused@example.com')
        await self.db.commit()
        self.assertEqual(list(await self.db.scalars(select(OAuthSession))), [])

    async def test_missing_account_is_404(self):
        with self.assertRaises(AccountDeletionError) as error:
            await delete_unassigned_account(self.db, 9999, 'unused@example.com')
...
class AccountDeletionApiTests(unittest.TestCase):
    def test_auth_confirmation_and_delete(self):
        ...
        with tempfile.TemporaryDirectory() as tmp, make_client(Path(tmp)) as client:
            self.assertEqual(client.request('DELETE', '/api/accounts/1', json={'confirmation_email':'unused@example.com'}).status_code, 401)
            ...
            self.assertEqual(client.request('DELETE', url, json={'confirmation_email':'wrong@example.com'}).status_code, 400)
            self.assertEqual(client.request('DELETE', url, json={'confirmation_email':'unused@example.com'}).status_code, 200)
            self.assertEqual(client.request('DELETE', url, json={'confirmation_email':'unused@example.com'}).status_code, 404)
