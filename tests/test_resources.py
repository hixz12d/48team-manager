...
from app.models import HmeAliasLease, Operation, PhoneAttempt, PhonePool, Team
...
class ResourceMigrationTests(unittest.TestCase):
    def test_run_auto_migration_creates_resource_tables_without_dropping_legacy(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            hme_cols = {row[1] for row in conn.execute("PRAGMA table_info(hme_alias_leases)").fetchall()}
            phone_cols = {row[1] for row in conn.execute("PRAGMA table_info(phone_pool)").fetchall()}
            op_cols = {row[1] for row in conn.execute("PRAGMA table_info(operations)").fetchall()}
            account_cols = {row[1] for row in conn.execute("PRAGMA table_info(accounts)").fetchall()}
            conn.close()

            self.assertIn("teams", tables)
            self.assertIn("phone_attempts", tables)
            self.assertIn("proxy_profiles", tables)
            self.assertIn("local_state", hme_cols)
            self.assertIn("operation_id", hme_cols)
            self.assertIn("label_sync_pending", hme_cols)
            self.assertIn("lease_heartbeat_at", phone_cols)
            self.assertIn("resolved_proxy", op_cols)
            self.assertIn("proxy_profile_id", account_cols)
...
class PhoneResourceTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_success_writes_attempt_and_counts(self):
        ...
        self.assertEqual(attempts[0].operation_public_id, "job-1")
    ...
    async def test_recently_used_cools_without_counting(self):
        ...
        attempts = list((await self.session.execute(select(PhoneAttempt))).scalars())
        self.assertEqual(attempts[0].result, OUTCOME_RECENTLY_USED)
    ...
    async def test_heartbeat_keeps_lease_alive_after_reserved_at_expires(self):
        ...
        stale = get_now() - timedelta(hours=1)
        row.reserved_at = stale
        row.lease_heartbeat_at = get_now()
...
class HmeResourceTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_email_otp_started_lease_is_not_reused_after_expiry(self):
        ...
        lease.expires_at = get_now() - timedelta(minutes=1)
        await self.session.commit()
        ...
        from app.services.hme import HmeConfig

        cfg = HmeConfig(base_url="http://icloud-hme:8081", service_token="token-token-token", account_id="acc_1")
        with patch("app.services.hme.load_config", AsyncMock(return_value=cfg)), patch(
    ...
    async def test_created_account_push_failure_consumes_alias(self):
        ...
        with patch("app.services.hme.apply_local_label", AsyncMock()) as tagged:
    ...
    async def test_label_failure_keeps_consumed_pending(self):
        ...
        await self.session.commit()
        claimed = self._claimed(lease)
        with patch("app.services.hme.apply_local_label", AsyncMock(side_effect=RuntimeError("label down"))):
        ...
        self.assertEqual(lease.label_desired, "星尘")
        leased = await active_leased_emails(self.session)
        self.assertIn("one@icloud.com", leased)
...
class ProxyFreezeTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_operation_keeps_frozen_proxy_after_page_change(self):
        ...
        team.proxy = "socks5h://10.0.0.9:1080"
        await self.session.commit()
