...
class TeamService:
    ...
    async def update_team(
        ...
        seat_cycle_days: Optional[int] = None,
    ) -> Dict[str, Any]:
        """
        更新 Team 信息
        ...
        try:
            ...
            if proxy is not None:
                ...
                team.proxy = normalize_proxy_url(cleaned) if cleaned else None
                await self.chatgpt_service.clear_session(team.email)

            if seat_cycle_days is not None:
                team.seat_cycle_days = max(1, int(seat_cycle_days))
